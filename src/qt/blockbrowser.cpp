...
#include <sstream>
#include <string>
double getBlockHardness(qint64 height)
{
    const CBlockIndex* blockindex = getBlockIndex(qint64 height);

    qint64 nShift = (blockindex->nBits >> 24) & 0xff;
...
qint64 getBlockHashrate(qint64 height)
{
    qint64 lookup = height;
...
{
    std::string strHash = getBlockHash(qint64 Height);
    uint256 hash(strHash);
...
std::string getBlockDebug(qint64 Height)
{
    std::string strHash = getBlockHash(qint64 Height);
    uint256 hash(strHash);
...
qint64 blocksInPastHours(qint64 hours)
{
    qint64 wayback = hours * 3600;
    bool check = true;
    qint64 height = pindexBest->nHeight;
    qint64 heightHour = pindexBest->nHeight;
    qint64 utime = (qint64)time(NULL);
    qint64 target = utime - wayback;
...
{    
    if(block)
    {
        ...
        qint64 Pawrate = getBlockHashrate(height);
        ...
        qint64 nBits = getBlocknBits(height);
        qint64 nNonce = getBlockNonce(height);
        qint64 atime = getBlockTime(height);
