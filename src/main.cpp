...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // colossuscoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // colossuscoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
if (pindexBest->nHeight == 1) { nSubsidy = 19000000000 * COIN; }
...
// colossuscoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes ColossusCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // colossuscoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // colossuscoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // colossuscoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // colossuscoin: track money supply and mint amount info
...
// colossuscoin: total coin age spent in transaction, in the unit of coin-days.
...
// colossuscoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // colossuscoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // colossuscoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // colossuscoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // colossuscoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // colossuscoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // colossuscoin: check proof-of-stake
    ...
    // colossuscoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // colossuscoin: getblocks may not obtain the ancestor block rejected
    ...
    // colossuscoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "ColossusCoin Test";
        CTransaction txNew;
        txNew.nTime = 1417034300;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
    ...
    if (true  && (block.GetHash() != hashGenesisBlock)) {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x8160b32248cae500cb6d6fbea163a6a9a105137c3d06824e1c15567450962651"));
        block.print();
        ...
        // colossuscoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x65, 0x42, 0x33, 0x07 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // colossuscoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // colossuscoin: tell downloading node about the latest block if it's
